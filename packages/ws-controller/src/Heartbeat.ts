...
export class Heartbeat {
  ...
  connectingXPromise: null | XPromise<any> = null;
  startTime: number = 0;
  ...
  constructor({ wsContoller, options }: { wsContoller: WsContoller, options?: HeartbeatConfig }) {
  ...
  send() {
    ...
    this.sendTimer = setTimeout(async () => {
      ...
      this.connectingXPromise = this.wsContoller._reExecute({ cb: () => this.wsContoller._wsConnect({}), retryCount: -1, intervalTime: 2000 });
  ...
  received(msg: MessageEvent) {
    ...
    const message = `heartbeat started at ${this.startTime}, completed in ${endTime}', duration is ${(endTime - this.startTime) / 1000} seconds`
    this.wsContoller.events.dispatchEvent('log', message)
